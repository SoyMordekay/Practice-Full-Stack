...
import { Transaction, TransactionStatus, CreateTransactionData } from '../../../../../../domain/entities/domain/entities/transaction.entity'
...
export class TransactionRepositoryPg implements ITransactionRepository {
  ...
  private toDomain(ormEntity: TransactionOrmEntity | null): Transaction | null {
    ...
    domainEntity.id = ormEntity.id;
    domainEntity.reference = ormEntity.reference;
    domainEntity.amountInCents = ormEntity.amountInCents;
    domainEntity.status = ormEntity.status as TransactionStatus;
    domainEntity.productId = ormEntity.productId;
    domainEntity.customerEmail = ormEntity.customerEmail;
  ...
  async updateStatus(id: string, status: TransactionStatus): Promise<Transaction> {
    ...
    if (!updatedOrmEntity) {
      throw new Error(`Failed to retrieve transaction with id "${id}" after update.`);
    }
